...
# ---------------------------------------------------------------------------------------------------------------
# Phase 23 unified entry point: everything from core.indicators2 (the Phase-12/13 extension set) is also reachable from this
# module, so callers only need one import path. Resolved lazily (PEP 562) to stay import-order safe: core.indicators2
# itself imports this module, so an eager re-export would see a half-initialised module.
def __getattr__(name):
    if name.startswith("_"):
        raise AttributeError(name)
    import importlib
    m = importlib.import_module("core.indicators2")
    try:
...
def __dir__():
    import importlib
    return sorted(set(globals()) | {k for k in dir(importlib.import_module("core.indicators2")) if not k.startswith("_")})
