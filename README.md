...
### Phase 11c — deeper chart understanding + offline analyst
* **`core/vision2.py`**: (1) `PatternDetector` — the "YOLO" idea done offline: HOG features + calibrated linear SVM trained on ~2 000 synthetic labelled charts (21 classes: H&S, double top/bottom, triangles, wedges, flags, channels, cup&handle, V, rounding, trends, range, none), 97–99 % hold-out; applied as a multi-scale sliding window over the *reconstructed* candles with NMS. Weights cached in `data/models/pattern_hog.pkl`. (2) `axis_digits` — OCR of the price axis with a template k-NN (no tesseract) + RANSAC line fit → **automatic price calibration** (BTC screenshot: 83 256 vs true 83 379; error < 1 %). (3) `overlays` — Hough detection of drawn trend-lines / horizontal levels and hue-traced indicator curves (MAs), with textual description. (4) `volume_pane` — attaches the volume histogram to the candles. (5) `understand(path)` — one call for everything.
* **`core/analyst.py`**: offline FinGPT-style layer: evidence collection from vision/patterns/forecast/sentiment/playbook → confluence score → grounded bull/bear brief with invalidation levels and R:R, plus intent-based Q&A ("روند چیه؟", "where is support?"). Optional real local LLM: drop a GGUF into `data/models` + `pip install llama-cpp-python` — same grounding prompt, no API.
* UI: Chart Vision shows the learned image patterns, overlays and the analyst brief, and has an **Ask** box; AI Desk has a new **Analyst** tab that runs the same pipeline on live data.

## Phase 12 — "make it actually profitable": realism, breadth, audit, portfolio, alerts

Everything below was added because the previous playbook was **over-optimistic**. Honest status is in the Health page.

| Problem found | Fix | Where |
|---|---|---|
| Only ~1000 bars per symbol (a few weeks of 1h) → any "proven" edge was one regime | Multi-year cache: 35 000 h1 bars (≈4 y), daily since listing/1980; incremental top-up (0.4 s) instead of full refetch; 86 symbols in 6 groups incl. indices/ETFs/DXY | `core/data.py` |
| Flat 5 bps commission + 2 bps slippage for everything | Per-bar **dynamic cost model**: half-spread × volatility ratio × session multiplier (FX rollover ×2.5, weekends, outside RTH) × thin-volume penalty; square-root market impact vs bar dollar-volume; funding/swap for leveraged positions; stop/market fills pay extra slippage | `core/costs.py` `dynamic_costs`, `core/backtest.py cost_model="dynamic"` |
| Strategies could peek at the future without anyone noticing | **Automatic audit** of all 100 strategies: truncation look-ahead test (3 cut points), determinism, signal sanity, speed and **random-walk test** (profit on data with no edge = leak). Result 98 pass · 2 warn · 0 fail; failures are excluded from Playbook/Advisor automatically; quick version in the test-suite | `core/audit.py`, `data/audit.json`, Health page |
| A strategy could be "proven" from one lucky symbol/year | **Breadth gates**: must be net positive on ≥60 % of the group's symbols and ≥60 % of calendar years; up to 8 sequential OOS folds; score weighted by breadth; 56 symbols in 4 groups | `core/playbook.py` |
| UI pickers offered 100 strategies, most unproven | "proven only ✓" toggle (default on when a playbook exists) on every strategy picker | `ui/pages.py SymbolBar` |
| No answer to "what should I actually run together" | **Portfolio page**: proven-only, greedy low-correlation selection of OOS equity curves, equal risk per member, combined curve / CAGR / max DD / Sharpe | `core/portfolio.py build_proven_portfolio`, `ui/portfolio_page.py` |
| Forward test needed the user to watch the screen | **Alerts**: Telegram Bot API + desktop tray balloons for new last-bar signals of proven strategies (auto every 15 min via maintenance thread, or "Scan now"); every alert is also recorded in the forward log | `core/alerts.py`, Settings → Alerts |
| Encyclopedia had 71 indicators with one-line "how to read" | **80 indicators**, each with parameters, *all* uses, concrete signal rules, pitfalls and best combinations (FA/EN) | `core/indicator_uses.py` |
